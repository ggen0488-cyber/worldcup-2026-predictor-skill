...
- 赛制与阶段：小组赛/联赛/积分赛、淘汰赛、附加赛、友谊赛或其他；记录是否允许平局、是否有加时/点球。
...
- `competition_context` 用于记录 Agent 搜索确认的赛制。积分赛使用 `format: "points"`；淘汰赛使用 `format: "knockout"` 并说明是否有加时和点球。若用户只给出两队名，必须先搜索确认赛制；无法确认时先问用户。
