...
description: Predict and simulate 2026 FIFA World Cup (USA/Canada/Mexico) matches, groups, champion odds, and knockout brackets after the AI Agent collects and saves current data and pre-match intelligence snapshots, then present the result as a local HTML report. Use when the user asks to predict "X vs Y", scorelines, group advancement, "谁会赢/谁会夺冠", tournament simulations, bracket paths, football randomness/upset scenarios, HTML prediction reports, or for-entertainment 算卦/玄学 predictions. Supports Agent-supplied live data, Agent-supplied team news/intelligence, Agent-supplied divination context, editable team ratings, stage-aware real-result locking, best-third-place advancement, built-in upset variance, local HTML report generation, and optional I Ching/Five Elements/zodiac/numerology output.
---

# 2026 世界杯预测 ⚽🔮

对 2026 美加墨世界杯做离线概率预测：单场比分、小组晋级、夺冠概率、淘汰赛路径。
默认使用**理性模型**（Elo → 波动修正泊松比分 → 蒙特卡洛）；用户明确要求时才叠加**玄学趣味模式**。
...
- `references/html_report.md` — HTML 报告内容结构、视觉要求与自动打开规则
